...
class Border:
    ...
    def calculate_vertices(self, rect):
        self.vertices = (rect.global_position[0] - self.width, rect.global_position[1] - self.width, rect.size[0] + self.width, rect.size[1] + self.width)
...
class Panel(Widget):
    def __init__(self, position, size, image, border=Border(0, (0, 0, 0)), text=Text('None', 0, Color.BLACK), parent=None):
        Widget.__init__(self, position, parent)
        self.enabled = True
        self.text = text
        self.size = size
